...
@section('body')
    ...
    <div class="login">
        ...
        <form action="{{ route('new-register.update', 200) }}" method="POST" class="form-horizontal pull-right w-100">
            @method('PATCH')
            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                ...
                <input type="text" class="form-control" id="email" name="mobile" value="{{ $user->mobile }}" readonly/>
            </div>

            <div class="form-group{{ $errors->has('password') ? ' has-error' : '' }}">
                <i class="fas fa-unlock-alt"></i>
                <input type="password" class="form-control" id="password" name="password" value="{{ $pass }}" readonly/>
            ...
            <div class="form-group{{ $errors->has('code') ? ' has-error' : '' }}">
                <i class="fas fa-unlock-alt"></i>
                <input type="number" class="form-control" id="code" name="code" placeholder="کد تایید را وارد کنید" value="{{ old('code') }}" autofocus/>
                {{-- @if ($errors->has('password'))
                    <span class="help-block"><strong>{{ $errors->first('password') }}</strong></span>
                @endif --}}
            </div>
