...
@section('body')
    <div class="condition pull-right w-100 mb-2">
        ...
        <form action="{{ route('admin-about-faq-update', $item->id) }}" method="POST"
              enctype="multipart/form-data" class="form-horizontal">
            <fieldset>
                <div class="form-group{{ $errors->has('place_id') ? ' has-error' : '' }}">
                    <div class="row">
                        <div class="col-md-12">
                            <label for="place_id" class="form-label">* محل :</label>
                            <select name="place_id" id="place_id" class="form-control">
                                <option value="1" {{$item->place_id==1?'selected':''}}>{{App\About::first()->title}}</option>
                                <option value="2" {{$item->place_id==2?'selected':''}}>{{App\About::first()->vision_title}}</option>
                            </select>
                            ...
                            <div class="form-group{{ $errors->has('text') ? ' has-error' : '' }}">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <textarea rows="3" class="form-control" id="text" name="text" required>{{ $item->text }}</textarea>
                                        @if ($errors->has('text'))
