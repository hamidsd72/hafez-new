@component('layouts.admin')
    ...
    @section('body')
        ...
        <div class="condition pull-right w-100 mb-2">
            ...
            <div class="p-3">
                <a href="{{ route('admin-about-feature-create') }}" class="btn btn-success mb-2"><i class="fa fa-plus ml-2"></i>افزودن ویژگی</a>
                <table class="table table-bordered pull-right w-100">
                    ...
                    @foreach($items as $item)
                        <tr>
                            <td>{{ $item->title }}</td>
                            <td><img src="{{url($item->pic)}}" style="height: 50px;background: rgba(0,0,0,0.1)" alt=""></td>
                            <td>
                                @if($item->status=='active') <span class="text-success">نمایش</span> @else <span class="text-danger">عدم نمایش</span> @endif
                                    @if($item->status == 'pending')
                                        <a href="{{route('admin-about-feature-active',['status',$item->id])}}" title="نمایش در سایت" style="margin-right: 10px"><i class="fa fa-check"
                                                                                                                             style="color: darkgreen;"></i></a>
                                    @endif
                                    @if($item->status == 'active')
                                        <a href="{{route('admin-about-feature-active',['status',$item->id])}}"
                            ...
                            <td>
                                ...
                                @if($item->status_home == 'pending')
                                    <a href="{{route('admin-about-feature-active',['status_home',$item->id])}}" title="نمایش در صفحه اصلی" style="margin-right: 10px"><i class="fa fa-check"
                                                                                                                         style="color: darkgreen;"></i></a>
                                @endif
                                @if($item->status_home == 'active')
                                    <a href="{{route('admin-about-feature-active',['status_home',$item->id])}}"
                            ...
                            <td class="table-td-icons" style="display: flex;justify-content: center;align-items: center;min-height: 80px">
                                <a href="{{ route('admin-about-feature-edit', $item->id) }}" title="ویرایش"><i
                                            class="far fa-edit"></i></a>

                                @role('ادمین ارشد')
                                <a href="{{route('admin-about-feature-destroy',$item->id)}}" title="حذف"  onclick="return confirm('برای حذف مطمئن هستید؟')"><i
