...
if ( !strpos( substr(  \Request::getPathInfo(), 1) , '/') ) {
    if ( strpos( \Request::getPathInfo() , '-') ) {
        if ( !in_array( \Request::getPathInfo() , ['/contact-us','/about-us','/sub-station'] ) ) {

            Route::get('{slug}', function ($slug) {
                $old_item = Redirect::where('old_slug', $slug)->first();
                if ($old_item) {
                    $item = Blog::find( $old_item->new_slug );
                } else {
                    $item = Blog::first();
                }
                $view = new PostsView();
                $view->post_id = $item->id;
                $view->save();
                $type = $item->type;
                $last_items=Blog::where('status','active')->where('type', $type)->where('id','!=',$item->id)->orderBy('id','desc')->take(4)->get();
                ...
                return view('user.blog.show',compact('type','item','last_items','comments','like','dislike'), ['title' => set_lang($item,'title',app()->getLocale())]);
            });
...
Route::group(['prefix' => 'admin', 'namespace' => 'App\Http\Controllers\Admin', 'middleware' => ['auth', 'Admin']], function () {
    ...
    Route::resource('admin-headers-links', 'HeadersLinksController');
    ...
    Route::get('employment-page-destroy/{id}', 'ContactController@destroy')->name('admin.employment.page.destroy');
    ...
    Route::get('blog-lists/{type}', 'BlogController@index')->name('admin-blog-list');
    ...
    //change pass
    Route::get('change-pass', 'HomeController@change_pass')->name('change-pass');
    ...
    Route::get('user_show_ip', 'UserController@show_ip')->name('user_show_ip');
    Route::post('user_edit_ip', 'UserController@edit_ip')->name('user_edit_ip');
...
Route::group(['prefix' => '', 'namespace' => 'App\Http\Controllers\User'], function () {
    ...
    Route::resource('new-register', 'NewRegisterController');
    ...
    Route::get('blog-post/search', 'BlogController@search')->name('user.blog.search');
